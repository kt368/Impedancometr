...
/*****************************************************************************//**
* @brief        Get SPI settings:
*               CPHA, CPOL, ClockRate, DataOrder, Databit, Mode
* @param[in]    SPIx    SPI peripheral selected, should be LPC_SPI
* @param[in]    SPI_InitStruct Pointer to a SPI_CFG_Type structure
*                    which will be initialized with current SPIx settings.
* @return       none

*******************************************************************************/
void SPI_GetConfig(LPC_SPI_TypeDef* SPIx, SPI_CFG_Type *SPI_GonfigStruct)
{
		uint32_t tmp;
		tmp = SPIx->SPCR;
    SPI_GonfigStruct->CPHA = tmp & (1<<3);
    SPI_GonfigStruct->CPOL = tmp & (1<<4);
    SPI_GonfigStruct->ClockRate = SPIx->SPCCR;
    SPI_GonfigStruct->DataOrder = tmp & (1<<6);
    SPI_GonfigStruct->Databit = (tmp >> 8) & 0xf;
    SPI_GonfigStruct->Mode = tmp & (1<<6);
}
